...
class ClientService {
  ...
  Future<List<dynamic>> fetchClientData({
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Erro ao carregar lista de clientes');
    }
  }
...
Future<void> updateClient({
  ...
  print("Corpo da solicitação: ${jsonEncode(requestData)}");
  ...
  try {
    ...
    if (response.statusCode != 200) {
      print('Erro ao atualizar cliente. Código de status: ${response.statusCode}');
      print('Corpo da resposta: ${utf8.decode(response.bodyBytes)}');
      throw Exception('Erro ao atualizar cliente: ${utf8.decode(response.bodyBytes)}');
    } else {
  ...
  Future<void> deleteClient({required int clientId, required int userId}) async {
    final url = Uri.parse('$baseUrl/client-address-contact/delete/$clientId/$userId');
