  ...
  class StackProposalEdicao extends StatefulWidget {
    ...
    const StackProposalEdicao({required this.proposalId});
  ...
  class _StackProposalEdicaoState extends State<StackProposalEdicao> {
    ...
    Future<void> fetchProposalForEdit() async {
      try {
        Map<String, dynamic> proposalData = await _proposalService.fetchProposalForEdit(widget.proposalId);
        setState(() {
          _leadId.text = proposalData['idLead'].toString();
          _date.text = DateFormat('dd/MM/yyyy').format(DateTime.parse(proposalData['completionDate']));
          _description.text = proposalData['description'];
          _service.text = proposalData['service'];
          _value.text = 'R\$ ${proposalData['value'].toString()}';
          _clientId.text = proposalData['clientId'].toString();
          _clientName.text = proposalData['clientName'];
          _selectedStatus = proposalData['idStatusProposal'];
        });
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Erro ao buscar detalhes da proposta: $e')),
        );
      }
    }
